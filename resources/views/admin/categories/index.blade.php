        ...
        <div>
            ...
            <x-table.table class="table-fixed" divClass="rounded-b-none">
                <x-table.thead>
                    <x-table.tr class="border-b-0">
                        <x-table.th class="w-[50px] px-0 text-center">
                            No
                        </x-table.th>
                        <x-table.sortable-th class="w-full" name="name">
                            Name
                        </x-table.sortable-th>
                        <x-table.sortable-th
                            class="w-[200px]"
            ...
            @foreach ($categories as $category)
                @if (! $category->parent_id)
                    <x-table.table
                        class="table-fixed"
                        divClass="rounded-t-none rounded-b-none border-b-0 border-t-0"
                    >
                        <x-table.tbody>
                            <x-table.tr class="border-b-0">
                    ...
                    <div class="">
                        <x-table.table
                            class="table-fixed"
                            divClass="rounded-t-none rounded-b-none border-b-0"
                        >
                            <x-table.tbody>
                                @foreach ($category->subCategories as $subCategory)
                                    <x-table.tr>
                                        ...
                                        <x-table.td class="w-full pl-[50px]">
                                            {{ $subCategory->name }}
                                        </x-table.td>
                                        <x-table.td class="w-[200px]">
                                            {{ $subCategory->created_at->format('M, j Y') }}
                                        </x-table.td>
                                        <x-table.actions-td class="w-[300px]">
                                            <x-table.view-button
