    ...
    <x-slot name="header">
        ...
        <div class="space-y-4 rounded-lg bg-white p-5 shadow">
            ...
            <x-show.item label="Parent Category">
                <x-dash.anchor-link href="{{ route('dashboard.categories.show', $parent) }}">{{ $parent->name }}</x-dash.anchor-link>
            </x-show.item>
            @endif
            @if (count($subCategories) !== 0)
            <x-show.item label="Sub categories" class="flex gap-2">
                @foreach ($subCategories as $key => $subCategory)
                    <x-dash.anchor-link href="{{ route('dashboard.categories.show', $subCategory) }}">{{ $key !== count($subCategories)-: ? "{$subCategory->name}, ": $subCategory->name }}</x-dash.anchor-link>
                @endforeach
            </x-show.item>
            @endif
        </div>
